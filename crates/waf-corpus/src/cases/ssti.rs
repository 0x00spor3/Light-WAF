...
pub static CASES: &[Case] = &[
    ...
    Case {
        id: "ssti-thymeleaf-selection-arith",
        module: Module::Ssti,
        field: Field::Query { name: "q", value: "*{7*7}" },
        min_pl: 1,
        expect: Expect::Triggers,
        rules: &["ssti-template-arithmetic"],
        desc: "E-4: Thymeleaf selection `*{7*7}` — new delimiter for the arithmetic rule",
    },
    Case {
        id: "ssti-razor-arith",
        module: Module::Ssti,
        field: Field::Query { name: "q", value: "@(1+2)" },
        min_pl: 1,
        expect: Expect::Triggers,
        rules: &["ssti-template-arithmetic"],
        desc: "E-4: Razor `@(1+2)` explicit expression",
    },
    Case {
        id: "ssti-jinja-print-paren",
        module: Module::Ssti,
        field: Field::Query { name: "q", value: "{%print(7*7)%}" },
        min_pl: 1,
        expect: Expect::Triggers,
        rules: &["ssti-template-statement"],
        desc: "E-4: `{%print(7*7)%}` — `(` directly after the keyword (was missed)",
    },
    Case {
        id: "ssti-smarty-php",
        module: Module::Ssti,
        field: Field::Query { name: "q", value: "{php}phpinfo();{/php}" },
        min_pl: 1,
        expect: Expect::Triggers,
        rules: &["ssti-smarty-php"],
        desc: "E-4: Smarty `{php}…{/php}` raw-PHP execution tag",
    },
    Case {
        id: "ssti-benign-razor-member",
        module: Module::Ssti,
        field: Field::Query { name: "tpl", value: "@model.Name" },
        min_pl: 1,
        expect: Expect::Clean,
        rules: &[],
        desc: "E-4 FP guard: Razor member access `@model.Name` — no delimiter+digit-op-digit",
    },
    Case {
        id: "ssti-benign-css-universal",
        module: Module::Ssti,
        field: Field::Query { name: "style", value: "*{margin:0}" },
        min_pl: 1,
        expect: Expect::Clean,
        rules: &[],
        desc: "E-4 FP guard: CSS universal selector `*{margin:0}` — no digit-op-digit after `*{`",
    },
];
